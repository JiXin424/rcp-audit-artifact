...
def build_text_nearest_registry(
    ...
    Donors whose NFKC-normalized text equals the query's NFKC-normalized
    text are excluded (exact-normalized-text exclusion, paper §2.2).

    Jaccard ties are resolved by character-level Levenshtein distance;
    Levenshtein ties are resolved by SHA-256 hash of the donor ID
    (two-level tie-breaking, paper §2.2).

    Args:
        queries: list of {'id', 'text'} for test set
        donors: list of {'id', 'text'} for train pool
        exclusion_threshold: if > 0, additionally exclude donors
            with Jaccard > threshold (donor-exclusion sweep).

    Returns:
        Dict {query_id: {'donor_id', 'jaccard', 'donor_text',
               'levenshtein', 'sha256_tb'}}
    """
    donor_tokens = [(d["id"], d["text"], token_set(d["text"]),
                     normalize_text(d["text"])) for d in donors]
        ...
        if best is None:
            ...
            for did, dtext, dtok, d_norm in donor_tokens:
                ...
                if jac > best_jac:
                    best_jac = jac
                    best = {"donor_id": did, "jaccard": jac, "donor_text": dtext,
                            "levenshtein": char_levenshtein(q_norm, d_norm),
                            "sha256_tb": None}
        registry[q["id"]] = best
    return registry
