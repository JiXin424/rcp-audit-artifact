...
Step-corrected & 2 & 2 & 2 & 2 & dose--resp., validation-frequency \\
...
\paragraph{Method.}
We fit the gap--competence dose--response over all 42 unique non-released weight binaries (the 43 decoded runs reduce to 42 after SHA-256 deduplication of the long-schedule/validation-freq-misread seed-202 duplicate), spanning dev BLEU-4 $0$--$9.8$ (37 checkpoints at $\geq 4.5$; the five degenerate checkpoints sit at dev $\approx 0$), using three models: OLS linear, OLS quadratic, and a Gaussian Process with RBF + white kernel. Each model predicts the PURE--REC gap at dev BLEU-4 $=13.38$.
...
\centering
\scriptsize\setlength{\tabcolsep}{3pt}
\caption{Canonical checkpoint registry (summary). 78 training runs across 13 families; 44 non-degenerate checkpoints enter the non-replication count, 49 have a canonical PHX-public PURE--REC gap, and the released evaluator is the audit target. Full per-checkpoint details in Sup.~D.}
...
This appendix documents the three sanity checks requested by the reviewer to rule out a systematic implementation defect shared across all 78 training runs. Scripts: \texttt{scripts/e\_overfit\_test.py}, \texttt{scripts/e\_checkpoint\_stats.py}, \texttt{scripts/e\_inference\_regression.py}. Results: \texttt{results/overfit\_test.json}, \texttt{results/checkpoint\_stats.json}, \texttt{results/inference\_regression.json}.
