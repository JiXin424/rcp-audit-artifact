...
\centering
\scriptsize\setlength{\tabcolsep}{3pt}
\caption{Three-layer reproduction of the headline results from the artifact (\S\ref{sec:method}). Layer 1 regenerates every paper number from the committed per-item decodes; layer 2 re-executes the analyses from committed per-checkpoint data; layer 3 retrains from the public recipe (requires the training compute).}
\label{tab:reproduction_layers}
\begin{tabular}{p{2.6cm}p{4.6cm}p{6.8cm}}
\toprule
Layer & Command & Scope \\
\midrule
L1: one-command & \texttt{make core-audit} & Regenerates the canonical donor registry and all headline numbers from committed per-item decodes (asserts REC 12.78 / PURE 23.02 / gap $+10.24$ / CI $[8.88, 11.62]$) \\
L2: scripted re-execution & \texttt{make round2-all}; individual \texttt{make} targets (donor-bootstrap, ref-frame-figure, experiment-C/A1/D, full-readout) & Re-runs every analysis from committed per-checkpoint data and hypotheses \\
L3: full retraining & \texttt{scripts/train\_reconstruction.py}, \texttt{launch\_reconstructions.sh}; rescue/distillation/config-faithful launchers & Retrains and re-evaluates from the public recipe; requires the training bundle (checkpoints + SLRTP2025 pose records) \\
\bottomrule
\end{tabular}
\end{table}
