...
The eight extension seeds are post-hoc additions that enter only the seed-level prediction interval and descriptive ranges, never the six primary comparisons. Per-seed PHX-public values (sacreBLEU 0--100 scale):

\begin{table}[h]
\centering
\scriptsize
\caption{Per-seed extension cells for the eight post-hoc extension seeds (707--1405). All values are PHX-public sacreBLEU on the 0--100 scale. These seeds reproduce the near-zero/negative gap pattern of the primary six and enter only the prediction interval, not the primary comparisons.}
\label{tab:extension_cells}
\begin{tabular}{lrrrrrrr}
\toprule
Seed & dev BLEU-4 & GT & PURE & Gap & COMP & Gap & WER \\
\midrule
707  & 6.69 & 7.71 & 7.55 & $-0.16$ & 7.21 & $-0.50$ & 93.00 \\
808  & 7.30 & 7.71 & 6.84 & $-0.87$ & 6.90 & $-0.81$ & 94.83 \\
909  & 8.96 & 9.56 & 9.09 & $-0.47$ & 8.83 & $-0.73$ & 88.70 \\
1001 & 9.07 & 8.63 & 8.36 & $-0.27$ & 8.19 & $-0.44$ & 89.21 \\
1102 & 7.61 & 8.88 & 7.78 & $-1.10$ & 7.85 & $-1.03$ & 91.92 \\
1203 & 8.14 & 9.12 & 8.53 & $-0.59$ & 8.53 & $-0.59$ & 90.44 \\
1304 & 8.18 & 9.41 & 8.62 & $-0.79$ & 8.58 & $-0.83$ & 90.77 \\
1405 & 8.12 & 8.90 & 8.17 & $-0.73$ & 8.37 & $-0.53$ & 90.51 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\scriptsize\setlength{\tabcolsep}{3pt}
\caption{Epoch-validation approximation (validate every 14 epochs, not steps): up to 3{,}000 epochs, dev-BLEU early stopping, patience 5, plateau $\times0.8$. All four early-stop below the competence gate; none shows the reversal or readout signature.}
...
Family range (n varies) & --- & 4.5--10.0 (n=52) & 86.5--96.7 (n=52) & $-1.10$ to $+0.25$ (n=24) & 1.05--1.31 (n=23) & 4.9--10.4 (n=23) \\
\bottomrule
\end{tabular}
\end{table}
...
The headline gap (+11.01) and the reference-attenuation result are reported with query-level paired bootstrap CIs. Because TN-PURE reuses donors and PHOENIX-2014T has template families and multiple signers, query-level resampling may underestimate uncertainty if dependence through shared donors, signers, or broadcast structure is non-negligible. Table~\ref{tab:cluster_bootstrap} reports interpretable cluster-aware resampling designs alongside the query-level baseline, using precomputed per-item n-gram sufficient statistics for 10{,}000 resamples each.

\begin{table}[h]
\centering
\scriptsize
\caption{Cluster-aware bootstrap for the headline PURE--GT gap (greedy decode, original refs). All CIs are 10{,}000-resample sacreBLEU \texttt{tok:13a} exp-smooth. Signer (9 clusters), broadcast show (2: \emph{heute}/\emph{tagesschau}), broadcast-date (297 clusters), and signer$\times$show (17 clusters) replace the previous 60-char prefix clustering (627 near-singleton clusters) with interpretable groupings. With so few clusters (especially show with only 2), these are \emph{exploratory sensitivity checks}, not confirmatory tests; leave-one-cluster-out or wild bootstrap would be needed for stronger inference.}
\label{tab:cluster_bootstrap}
\begin{tabular}{lrrrr}
\toprule
Analysis & Design & Clusters & Gap & 95\% CI \\
\midrule
Orig-ref gap (greedy) & Query & 641 & $+9.89$ & $[+8.45, +11.40]$ \\
& Signer & 9 & $+9.93$ & $[+9.08, +10.84]$ \\
& Show & 2 & $+9.26$ & $[+6.56, +10.73]$ \\
& Date & 297 & $+9.90$ & $[+8.38, +11.45]$ \\
& Signer$\times$Show & 17 & $+9.81$ & $[+7.95, +11.40]$ \\
\bottomrule
\end{tabular}
\end{table}
