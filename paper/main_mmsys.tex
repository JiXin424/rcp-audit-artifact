...
We address this question in four stages. First, we compare the reversal with reference-pose, generative and random-donor controls under the original evaluator, verifying protocol details (scorer equivalence, frame-rate handling, decoding parameters) and aligning all secondary text metrics with the official repository implementations. Second, we evaluate the same fixed outputs with fourteen independently trained recipe-conditioned reconstructions and quantify paired uncertainty, documenting that no rescue variant or any of the three examined checkpoint-selection objectives (NLL, decoded BLEU, decoded WER) reaches competence parity. Third, we test reference validity directly: replacing the speech-transcribed PHOENIX references with public human sign-to-text back-translations~\cite{czehmann2026sacred} attenuates the reversal from $+11.01$ to $+1.12$ BLEU points, and a reference-anchored weather-slot diagnostic shows the replay's decoded content tracks the donor transcript. Fourth, we characterize what distinguishes the released pipeline: donor-pool substitution designs (path-dependent, so reported as descriptive contrasts alongside a balanced 605-query factorial), and five related diagnostic views of a training-content readout property --- a training-pool readout signature combined with generalization that no reconstruction exhibits, donor-transcript pass-through far above the family trend, temporal-order sensitivity confined to the released evaluator, a flat gap--competence dose--response, and a frozen-protocol confirmation on two fresh seeds. Evaluator-aware oracle analyses are relegated to the appendix because their unmasked advantage does not survive reference-overlap exclusion. Together, these analyses distinguish a fixed-checkpoint, reference-dependent observation from a checkpoint-robust conclusion (Fig.~\ref{fig:audit_framework}).
...
\paragraph{Memorization \emph{with} generalization: the released evaluator's distinctive combination.}
\label{sec:insample}
...
\paragraph{Synthesis.}
Across the five related diagnostic views the released evaluator is not merely a better recognizer from the same family: it shows a readout-with-generalization signature (\S\ref{sec:insample}) and reproduces donor transcripts at 2.4$\times$ the family trend, and its replay advantage tracks that readout. The recipe-conditioned family, trained on identical data, shows none of these properties at any reachable competence. The reversal is therefore best described as a property of the released checkpoint and its data pipeline, co-occurring with unusually strong train-pool readout under the original reference frame --- the specific mechanism is not identifiable --- and not a property of BT evaluation generally or evidence that a randomly reseeded replica would behave alike.

\paragraph{Step-faithful re-run (correct protocol).}
\label{sec:stepfaithful}
Because the epoch-validation approximation of \S\ref{sec:config_faithful_results} deviates from the released configuration's step-based validation frequency by a factor of $\approx$28, we additionally ran two seeds under the corrected protocol (validate every 14 optimizer steps, patience 5 validations, decrease\_factor 0.8, learning\_rate\_min $10^{-8}$, stop at LR minimum). At the time of writing, the step-faithful runs (2 seeds, seeds 101 and 202) have reached $\approx$1{,}000 optimizer steps ($\approx$38 epochs): best dev BLEU-4 is 6.95 (seed 101) and 6.81 (seed 202), both at step $\approx$854, whereas the released evaluator reached 10.01 by step 770 on the same protocol. The reconstruction trajectory diverges from the original after step $\approx$600, where the original accelerates (8.32 $\to$ 10.01 between steps 616 and 770) while the reconstruction plateaus ($\approx$6--7). Training is continuing to the LR-minimum stop condition; final values will be reported in the artifact. The interim result already shows that the step-corrected protocol does not close the trajectory gap, though it does validate more frequently and select checkpoints more finely than the epoch-approximation runs above.
