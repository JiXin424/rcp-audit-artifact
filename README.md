...
├── artifact_ledger.json           # SHA-256 ledger (396 files; auto-generated from current artifact state)
├── manifests/
...
- **Donor registry:** The paper's retrieval algorithm (NFKC + Levenshtein + SHA-256 + exact-normalized-text exclusion) produces identical decoded output to the earlier frozen materialization on 610/641 items (95.2%). On the 31 differing items, a post-hoc candidate search found that 30 had at least one excluded Jaccard=1 candidate whose decode matched the earlier hypothesis; one item remained unresolved. This is consistent with the exclusion rule explaining most differences but does not uniquely recover the historical donor registry. See `results/hypothesis_comparison.jsonl` and `results/donor_identification_31.json`.
